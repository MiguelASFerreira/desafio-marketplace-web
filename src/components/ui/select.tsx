...
>(({ className, children, LeftIcon, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    {...props}
  >
    {LeftIcon && <LeftIcon data-icon className="mr-2 h-6 w-6" />}
    {children}
    <SelectPrimitive.Icon asChild>
      <ArrowDown01Icon className="ml-auto h-6 w-6 !text-gray-400" />
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex w-full cursor-default select-none items-center rounded-sm p-4 text-sm outline-none focus:text-primary data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
      className,
    )}
    {...props}
  >
    <span className="absolute right-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Tick02Icon className="h-6 w-6 text-primary" />
      </SelectPrimitive.ItemIndicator>
    </span>
